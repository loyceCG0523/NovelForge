# Windows 交互性能记录（0.2.5 / 0.2.6 / 0.2.7）

## 0.2.7：所有正式安装默认软件渲染

0.2.6 安装版在 RTX 4070 Laptop 与虚拟显示适配器共存的环境中仍出现侧栏纹理斜向撕裂和棋盘格缺块。安装版 EXE 与构建产物、Qt 6.11.1 WebEngine DLL 的哈希一致，因此不是开发模式或安装漏包；故障仍发生在 Chromium 已完成绘制之后的 Qt/DWM 呈现层。
...
| --- | --- | ---: |
| 默认 GPU | mixed，32 次 | 0 |
| `--disable-gpu-compositing` | mixed，32 次 | 1（显式重绘后恢复） |
| `--disable-gpu` | mixed，16 次 | 0 |
| `--disable-direct-composition` | mixed 64 次 + toggle 64 次 | 0 |

默认 GPU 路径属于低频故障，不能用一次未复现来证明适配所有机器；关闭 DirectComposition 虽然表现良好，仍保留硬件相关路径。为保证独显、核显、远程桌面、虚拟显示和基础显示驱动环境都走同一条可预测的通路，正式应用启动时无条件加入 `--disable-gpu`。`--safe-rendering` 保留为旧快捷方式的兼容参数，但不再改变实际模式。

这项选择以交互性能换取呈现一致性。GPU 路径仅保留在诊断脚本中，用于后续评估 Qt WebEngine 或桌面壳替换方案。
