...
from app.schemas.story_bible import (
    StoryBibleGenerateRequest,
    StoryBibleRead,
    StoryBibleSummaryRead,
    StoryBibleUpdate,
)
...
def _get_or_create_story_bible(db: Session, novel: Novel) -> StoryBible:
    """Return the bible, creating the same backwards-compatible draft if absent."""
...
@router.get("/summary", response_model=StoryBibleSummaryRead)
def get_story_bible_summary(
    novel: Novel = Depends(get_owned_novel),
    db: Session = Depends(get_db),
) -> StoryBible:
    """读取作品页所需的轻量状态，不传输大型结构化 content。"""
    return _get_or_create_story_bible(db, novel)
