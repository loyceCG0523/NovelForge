...
def _chapter_event_plan_payload(plan: EventChapterPlan, story_event_title: str = "") -> dict:
    return {
        "story_event_id": str(plan.story_event_id),
        "story_event_title": story_event_title,
        "plan_id": str(plan.id),
...
def _chapter_event_plan(db: Session, chapter: Chapter) -> dict | None:
    ...
    return _chapter_event_plan_payload(plan, story_event.title if story_event else "")


def _chapter_event_plans(db: Session, novel_id: UUID) -> dict[int, dict]:
    """Load each chapter's latest event plan and event title in one query."""
    rows = db.execute(
        select(EventChapterPlan, StoryEvent.title)
        .outerjoin(StoryEvent, StoryEvent.id == EventChapterPlan.story_event_id)
        .where(EventChapterPlan.novel_id == novel_id)
        .order_by(
            EventChapterPlan.chapter_index.asc(),
            EventChapterPlan.updated_at.desc(),
        )
    ).all()
    plans_by_chapter: dict[int, dict] = {}
    for plan, story_event_title in rows:
        plans_by_chapter.setdefault(
            plan.chapter_index,
            _chapter_event_plan_payload(plan, story_event_title or ""),
        )
    return plans_by_chapter


def _chapter_to_read(
    db: Session,
    chapter: Chapter,
    *,
    event_plans: dict[int, dict] | None = None,
    ...
    return {
        ...
        "context_snapshot": chapter.context_snapshot if include_content else {},
        "event_plan": (
            _chapter_event_plan(db, chapter)
            if event_plans is None
            else event_plans.get(chapter.chapter_index)
        ),
    }
...
def list_chapters(
    ...
    chapters = list(db.scalars(statement).all())
    event_plans = _chapter_event_plans(db, novel_id) if chapters else {}
    return [
        _chapter_to_read(
            db,
            chapter,
