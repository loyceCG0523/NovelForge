...
def build_chapter_meme_pack(
    ...
    *,
    ...
    if db.get_bind().dialect.name == "sqlite":
        candidates = db.scalars(select(MemeEntry).where(*filters)).all()
        vector_rows = sorted(
            ((entry, 1.0 - cosine_similarity(entry.embedding, query_vector)) for entry in candidates),
            key=lambda row: row[1],
        )[:30]
        lexical_rows = sorted(
            ((entry, text_similarity(entry.retrieval_text, query_text[:900])) for entry in candidates),
            key=lambda row: row[1],
            reverse=True,
        )[:20]
    else:
