...
def push_task_to_queue(task_id: str, queue_name: str | None = None) -> None:
    ...
    submit_local_task(
        task_id,
        memory=(queue_name or settings.agent_task_queue) == settings.memory_task_queue,
    )
...
def enqueue_agent_task(db: Session, novel: Novel, payload: AgentRunRequest) -> GenerationTask:
    ...
    task = GenerationTask(
        ...
        result_payload={
            ...
            "agent": "LocalTaskQueue",
            "note": "任务已进入本地队列，内置 Worker 会按任务类型分发到处理器或 LangGraph 工作流。",
        },
    )
    db.add(task)
    db.commit()
...
def enqueue_standalone_agent_task(db: Session, payload: AgentRunRequest) -> GenerationTask:
    ...
    task = GenerationTask(
        ...
        result_payload={
            "input": payload.input_payload,
            "agent": "LocalTaskQueue",
            "note": "独立任务已进入本地队列，内置 Worker 会分发到对应处理器执行。",
        },
    )
    db.add(task)
    db.commit()
