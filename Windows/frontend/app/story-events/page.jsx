...
import { apiFetch, apiFetchCached, getCachedApiData, isTaskInFlight, isTaskSettled } from "@/lib/api";
...
function StoryEventsContent() {
  ...
  const [selectedNovelId, setSelectedNovelId] = useState(() => novelIdFromUrl || initialProjects?.[0]?.id || "");
  ...
  async function loadProjects() {
    ...
    setProjects(data);
    const nextNovelId = novelIdFromUrl || selectedNovelId || data[0]?.id || "";
    setSelectedNovelId(nextNovelId);
    return nextNovelId;
  }

  async function loadEvents(novelId, preferredEventId = eventIdFromUrl, { force = false } = {}) {
    if (!novelId) return "";
    const data = await apiFetchCached(`/api/novels/${novelId}/story-events`, { ttlMs: 30_000, force });
    setEvents(data);
    const nextEventId = (
      (preferredEventId && data.some((item) => item.id === preferredEventId) && preferredEventId)
      || (selectedEventId && data.some((item) => item.id === selectedEventId) && selectedEventId)
      || data[0]?.id
      || ""
    );
  ...
  async function loadEventDetail(novelId, eventId, { force = false } = {}) {
    if (!novelId || !eventId) {
      setEventDetail(null);
      return;
    }
    setEventDetail(await apiFetchCached(`/api/novels/${novelId}/story-events/${eventId}`, { ttlMs: 30_000, force }));
  }
  ...
  useEffect(() => {
    ...
    setSelectedEventId("");
    setEventDetail(null);
    loadEvents(selectedNovelId, eventIdFromUrl).catch((err) => setError(err.message));
  }, [eventIdFromUrl, selectedNovelId]);

  useEffect(() => {
    loadEventDetail(selectedNovelId, selectedEventId).catch((err) => setError(err.message));
  }, [selectedEventId, selectedNovelId]);
  ...
  useLiveRefresh({
    ...
    refresh: async () => {
      ...
      if (isTaskSettled(task.status)) {
        await loadEvents(selectedNovelId, undefined, { force: true });
        await loadEventDetail(selectedNovelId, selectedEventId, { force: true });
      } else {
        await loadEventDetail(selectedNovelId, selectedEventId, { force: true });
      }
      setError("");
...
export default function StoryEventsPage() {
  return (
    <Suspense fallback={<div className="route-loading">正在载入剧情事件...</div>}>
      <StoryEventsContent />
    </Suspense>
  );
}
