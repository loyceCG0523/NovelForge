...
import { apiFetch, apiFetchCached, getStoredUser, isTaskInFlight, subscribeSse } from "@/lib/api";
...
const DAILY_ANNOTATION_NOTICE = "标注已保存，并会作为本地创作检索经验使用。";
...
function SampleAnalysisContent() {
  ...
  async function loadWorks({ force = false } = {}) {
    const data = uniqueById(await apiFetchCached("/api/sample-analyses", { ttlMs: 30_000, force }));
    setWorks(data);
  ...
  useEffect(() => {
    Promise.all([
      loadWorks(),
      apiFetchCached("/api/sample-collaboration/categories", { ttlMs: 300_000 }).then(setCategories)
  ...
  useLiveRefresh({
    ...
    intervalMs: 2500,
    refresh: async () => {
      const data = await loadWorks({ force: true });
      if (!selectedWork?.id) return;
  ...
  return (
    <AppShell title="本地样本分析" actions={<span className="tag purple">AI 分析 · 人工标注 · 本地检索</span>}>
      {(message || error) ? <div className={error ? "error-box" : "hint-panel"}>{error || message}</div> : null}

      <section className="collab-library-toolbar">
        <div className="collab-tabs" role="tablist" aria-label="本地样本库">
          <button className="active" type="button">本地样本库</button>
        </div>
        <span className="panel-subtitle">样本文本、分析结果和人工标注只保存在当前电脑</span>
      </section>

      {scope === "mine" ? (
        <details className="panel collab-collapsible-panel">
          <summary>
            <span><strong>上传新样本</strong><small>支持 TXT、MD；仅保存在当前电脑</small></span>
            <span>展开</span>
          </summary>
          <form className="panel-body collab-upload-form" onSubmit={createWork}>
            ...
            <div className="collab-upload-action-row">
              ...
              <button className="primary-button collab-upload-submit" disabled={loading || !selectedFile}>{loading ? "提交中..." : "上传并分析"}</button>
            </div>
          </form>
        </details>
      ) : null}
      ...
      <section className="panel collab-library-panel">
        <div className="panel-header">
          <div><div className="panel-title">本地样本</div><div className="panel-subtitle">{works.length} 部作品，打开后可查看分析或进入沉浸标注</div></div>
        </div>
        <div className="panel-body">
          {!works.length ? <EmptyState title="暂无样本" description="上传 TXT 或 Markdown 后，即可进行 AI 分析和本地标注。" /> : (
              ...
              {works.map((work) => (
                <article className={`collab-work-card ${selectedWork?.id === work.id ? "active" : ""}`} key={work.id}>
                  <button type="button" className="collab-work-open" onClick={() => openWork(work).catch((err) => setError(err.message))}>
                    <span className="collab-card-status">{statusLabels[work.status] || work.status} · 本地</span>
        ...
        <section className="panel collab-work-overview">
          <div className="panel-header collab-overview-header">
            ...
            <div className="collab-overview-actions">
              ...
              {scope === "mine" ? <button className="danger-button" type="button" onClick={() => deleteWork(selectedWork).catch((err) => setError(err.message))}>删除</button> : null}
