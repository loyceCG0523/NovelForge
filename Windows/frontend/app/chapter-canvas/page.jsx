...
import { apiFetchCached, getCachedApiData } from "@/lib/api";
...
function ChapterCanvasContent() {
  ...
  const initialChapters = initialNovelId ? getCachedApiData(chapterListPath(initialNovelId)) : undefined;
  ...
  const isChapterDetailLoading = Boolean(selectedChapterId && !selectedChapter?.__detailLoaded);
  ...
  async function loadProjects() {
    ...
    setProjects(data);
    const nextNovelId = requestedNovelId || selectedNovelId || data[0]?.id || "";
  ...
  async function loadChapters(novelId) {
    ...
    const data = await apiFetchCached(chapterListPath(novelId), { ttlMs: 15_000 });
    if (requestId !== chapterListRequestRef.current) return;
  ...
  async function loadChapterDetail(novelId, chapterId) {
    ...
    setChapters((current) => current.map((chapter) => (
      chapter.id === chapterId ? { ...chapter, ...detail, __detailLoaded: true } : chapter
    )));
  }
  ...
  useEffect(() => {
    if (!selectedNovelId || !selectedChapterId || selectedChapter?.__detailLoaded) return;
                ...
                <>
                  <header className="canvas-head">
                    ...
                    <div className="canvas-meta">
                      ...
                      <span>上下文：{isChapterDetailLoading ? "读取中" : selectedChapter.context_snapshot?.schema_version ? "已保存" : "暂无"}</span>
                    </div>
                  </header>
...
export default function ChapterCanvasPage() {
  return (
    <Suspense fallback={<div className="route-loading">正在载入章节画布...</div>}>
