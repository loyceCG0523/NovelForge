...
import { apiDownload, apiFetch, apiFetchCached, buildTimestampedDownloadFilename, getCachedApiData, isTaskInFlight } from "@/lib/api";
...
export default function ProjectsPage() {
  ...
  const [storyBibleSummaryExpanded, setStoryBibleSummaryExpanded] = useState(false);
  ...
  const storyBibleSummary = storyBible?.summary
    || "保存起始需求文档后，系统会自动整理内部创作设定，用于后续剧情事件和章节生成。";
  const storyBibleSummaryIsLong = storyBibleSummary.length > 320;
  const visibleStoryBibleSummary = storyBibleSummaryIsLong && !storyBibleSummaryExpanded
    ? `${storyBibleSummary.slice(0, 320).trimEnd()}…`
    : storyBibleSummary;
  ...
  useEffect(() => {
    ...
    setProjectError("");
    setStoryBibleError("");
    setStoryBibleSummaryExpanded(false);
    setBriefDocMessage("");
  ...
  async function loadSampleLibrary() {
    try {
      const data = await apiFetchCached("/api/sample-analyses/library", { ttlMs: 300_000 });
  ...
  async function loadStoryBible(projectId) {
    setStoryBibleError("");
    const data = await apiFetchCached(`/api/novels/${projectId}/story-bible/summary`, { ttlMs: 30_000 });
    setStoryBible(data);
  }

  async function loadActiveStoryBibleTask(projectId) {
    const [runningTasks, queuedTasks] = await Promise.all([
      apiFetchCached(`/api/novels/${projectId}/tasks?status_filter=running`, { ttlMs: 30_000 }),
      apiFetchCached(`/api/novels/${projectId}/tasks?status_filter=queued`, { ttlMs: 30_000 })
    ]);
          ...
          <section className="panel">
            <div className="panel-header">
              ...
              <div className="inline-actions">
                ...
                {briefEditorOpen ? (
                  <>
                    <button type="button" className="ghost-button" onClick={() => setBriefEditorOpen(false)}>收起编辑</button>
            ...
            <div className="panel-body">
              ...
              {!selectedProject ? (
                ...
                <div className="project-overview">
                  <div>
                    <div className="project-type">{briefForm.genre || "未分类"} · {briefForm.work_type || "小说"}</div>
                    <h2>{briefForm.title || selectedProject.title}</h2>
                    <p>{briefForm.premise || "暂无一句话需求。"}</p>
                  </div>
                  <div className="project-meta-grid">
                    <div><span>目标字数</span><strong>{Number(briefForm.target_words || 0).toLocaleString()}</strong></div>
                    <div><span>人物数</span><strong>{Array.isArray(briefForm.characters) ? briefForm.characters.length : 0}</strong></div>
                    <div><span>故事年代</span><strong>{briefForm.story_era || "未填写"}</strong></div>
                    <div><span>参考样本</span><strong>{Array.isArray(briefForm.sample_reference_ids) ? briefForm.sample_reference_ids.length : 0}</strong></div>
                  </div>
                  <div className="inline-actions">
                    <button type="button" className="primary-button" onClick={() => setBriefEditorOpen(true)}>编辑起始需求</button>
                  </div>
                </div>
              ) : (
              ...
              {!selectedProject ? (
                ...
                <div className="setting-status-grid">
                  <div className="setting-status-main">
                    ...
                    <h2>{selectedProject.title}</h2>
                    <p>{visibleStoryBibleSummary}</p>
                    {storyBibleSummaryIsLong ? (
                      <button
                        type="button"
                        className="ghost-button"
                        onClick={() => setStoryBibleSummaryExpanded((expanded) => !expanded)}
                      >
                        {storyBibleSummaryExpanded ? "收起摘要" : "展开完整摘要"}
                      </button>
                    ) : null}
...
function BriefForm({ form, onChange, sampleLibrary = [] }) {
  ...
  const characters = Array.isArray(form.characters) ? form.characters : [];
  // Large projects can contain many long character biographies. Keep the
  // lightweight summaries mounted during route entry and create the editor
  // controls only after the user explicitly asks to edit one character.
  const [expandedCharacterKey, setExpandedCharacterKey] = useState("");

  useEffect(() => {
    if (
      expandedCharacterKey
      && !characters.some((character) => character.key === expandedCharacterKey)
    ) {
      setExpandedCharacterKey("");
    }
  }, [characters, expandedCharacterKey]);
  ...
  function addCharacter() {
    ...
    setExpandedCharacterKey(character.key);
    onChange("characters", [...characters, character]);
  }
                ...
                <div className="structured-card-head">
                  ...
                  <div className="structured-card-actions">
                    <button
                      ...
                      aria-expanded={expandedCharacterKey === character.key}
                      onClick={() => setExpandedCharacterKey((current) => (
                        current === character.key ? "" : character.key
                      ))}
                    >
                      {expandedCharacterKey === character.key ? "收起" : "编辑"}
                    </button>
                    <button type="button" className="text-danger-button" onClick={() => removeCharacter(character.key)}>删除</button>
                  </div>
                </div>
                {expandedCharacterKey === character.key ? (
                  <>
                  ...
                  <div className="structured-card-summary">
                    <span>{character.gender || "未填性别"}</span>
                    <span>{character.age || "未填年龄"}</span>
                    <span>{character.occupation || "未填职业"}</span>
                    {character.is_protagonist ? <span className="tag green">主角</span> : null}
                  </div>
                )}
              </article>
            ))}
