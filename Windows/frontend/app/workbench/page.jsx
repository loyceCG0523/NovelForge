...
import { apiFetch, apiFetchCached, getCachedApiData, isTaskInFlight } from "@/lib/api";
...
function WorkbenchContent() {
  ...
  async function loadDashboard(id, { force = false } = {}) {
    // Dashboard 由后端聚合，避免前端同时请求章节、任务、风险等多个接口。
    if (!id) return;
    setDashboard(await apiFetchCached(`/api/novels/${id}/dashboard`, { ttlMs: 30_000, force }));
  }
  ...
  useLiveRefresh({
    ...
    intervalMs: 1800,
    refresh: () => loadDashboard(selectedId, { force: true }),
  ...
  useLiveRefresh({
    ...
    refresh: async () => {
      ...
      await loadDashboard(selectedId, { force: true });
      setError("");
...
export default function WorkbenchPage() {
  return (
    <Suspense fallback={<div className="route-loading">正在载入创作工作台...</div>}>
      <WorkbenchContent />
    </Suspense>
  );
}
