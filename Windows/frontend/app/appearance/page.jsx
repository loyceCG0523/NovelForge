...
import { apiFetch, apiFetchCached, getStoredUser, setStoredUser } from "@/lib/api";
...
export default function AppearancePage() {
  ...
  useEffect(() => {
    ...
    apiFetchCached("/api/auth/me", { ttlMs: 60_000 })
      ...
      .catch((err) => {
        if (active) setError(`本地外观偏好读取失败：${err.message}`);
      });
    return () => {
      active = false;
    };
  ...
  async function chooseTheme(theme) {
    ...
    try {
      ...
      setStoredUser(user);
      setMessage(`已切换为 ${THEMES.find((item) => item.id === theme)?.name} 主题，并保存到本机。`);
    } catch (err) {
