...
function chapterListPath(novelId) {
  return `/api/novels/${novelId}/chapters?include_content=false`;
}

function withChapterDetail(chapter) {
  return { ...chapter, __detailLoaded: true };
}
...
function ChaptersContent() {
  ...
  const requestedNovelId = searchParams.get("novel") || "";
  const initialProjects = getCachedApiData("/api/novels");
  const initialNovelId = requestedNovelId || initialProjects?.[0]?.id || "";
  const initialChapters = initialNovelId
    ? getCachedApiData(chapterListPath(initialNovelId))
    : undefined;
  const initialChapter = initialChapters?.[0];
  const [projects, setProjects] = useState(() => initialProjects || []);
  const [projectsLoaded, setProjectsLoaded] = useState(() => initialProjects !== undefined);
  const [selectedNovelId, setSelectedNovelId] = useState(() => initialNovelId);
  const [chapters, setChapters] = useState(() => initialChapters || []);
  const [chaptersLoadedFor, setChaptersLoadedFor] = useState(() => initialChapters !== undefined ? initialNovelId : "");
  const [selectedChapterId, setSelectedChapterId] = useState(() => initialChapter?.id || "");
  const [draft, setDraft] = useState(() => initialChapter || emptyChapter);
  ...
  const chapterIssueRequestRef = useRef(0);
  ...
  const isChapterDetailLoading = Boolean(
    selectedChapterId && (draft.id !== selectedChapterId || !draft.__detailLoaded)
  );
  ...
  async function loadProjects() {
    ...
    setSelectedNovelId((current) => {
      if (requestedNovelId && data.some((project) => project.id === requestedNovelId)) return requestedNovelId;
  ...
  async function loadChapters(novelId, preferredChapterId = "") {
    ...
    const path = chapterListPath(novelId);
    const data = await apiFetchCached(path, { ttlMs: 15_000 });
    if (requestId !== chapterRequestRef.current) return;
    setChapters((current) => data.map((summary) => {
      const detailed = current.find((chapter) => chapter.id === summary.id && chapter.__detailLoaded);
      return detailed ? { ...summary, ...detailed } : summary;
    }));
    setChaptersLoadedFor(novelId);
    const nextChapter = data.find((chapter) => chapter.id === preferredChapterId) || data[0];
    setSelectedChapterId(nextChapter?.id || "");
    setDraft((current) => {
      if (nextChapter && current.id === nextChapter.id && current.__detailLoaded) return current;
      return nextChapter || { ...emptyChapter, chapter_index: data.length + 1 };
    });
    return nextChapter;
  }

  async function loadChapterDetail(novelId, chapterId) {
    if (!novelId || !chapterId) return;
    const requestId = ++chapterDetailRequestRef.current;
    const detail = await apiFetchCached(`/api/novels/${novelId}/chapters/${chapterId}`, { ttlMs: 15_000 });
    if (requestId !== chapterDetailRequestRef.current) return;
    const detailedChapter = withChapterDetail(detail);
    setChapters((current) => current.map((chapter) => (
      chapter.id === chapterId ? { ...chapter, ...detailedChapter } : chapter
    )));
    setDraft(detailedChapter);
  }

  async function loadChapterIssues(novelId, chapterId, { force = false } = {}) {
    const requestId = ++chapterIssueRequestRef.current;
    if (!novelId || !chapterId) {
      setChapterIssues([]);
      return;
    }
    // Tab revisits within a few seconds reuse the last response instead of
    // re-hitting the API on every page mount.
    const data = await apiFetchCached(`/api/novels/${novelId}/reviews?chapter_id=${chapterId}`, {
      ttlMs: 8_000,
      force
    });
    if (requestId === chapterIssueRequestRef.current) setChapterIssues(data);
  }
  ...
  useEffect(() => {
    if (!projectsLoaded || !selectedNovelId) return;
    if (chaptersLoadedFor !== selectedNovelId) {
      setChapters([]);
      setSelectedChapterId("");
      setDraft(emptyChapter);
    }
    loadChapters(selectedNovelId).catch((err) => setError(err.message));
  }, [projectsLoaded, selectedNovelId]);

  useEffect(() => {
    setContentExpanded(false);
    setContextExpanded(false);
    if (selectedChapterId) {
      if (selectedChapter?.__detailLoaded) {
        setDraft(selectedChapter);
      } else {
        if (selectedChapter) setDraft(selectedChapter);
        loadChapterDetail(selectedNovelId, selectedChapterId).catch((err) => setError(err.message));
      }
    }
    loadChapterIssues(selectedNovelId, selectedChapterId).catch((err) => setError(err.message));
  ...
  async function saveChapter(event) {
    ...
    try {
      const wasExistingChapter = Boolean(selectedChapterId);
      ...
      const detailedChapter = withChapterDetail(saved);
      setChapters((current) => {
        const exists = current.some((chapter) => chapter.id === saved.id);
        if (!exists) return [...current, detailedChapter];
        return current.map((chapter) => chapter.id === saved.id ? { ...chapter, ...detailedChapter } : chapter);
      });
      setDraft(detailedChapter);
      setSelectedChapterId(saved.id);
      await loadChapters(selectedNovelId, saved.id);
      if (wasExistingChapter) await loadChapterIssues(selectedNovelId, saved.id, { force: true });
    } catch (err) {
      setError(err.message);
    }
  }
  ...
  return (
    <AppShell
      ...
      actions={
        <>
          <select className="secondary-button" value={selectedNovelId} onChange={(event) => {
    ...
    >
      {!projectsLoaded ? (
        <section className="panel page-loading-panel" aria-busy="true">
          <div className="loading-spinner" />
          <strong>正在读取本地作品…</strong>
        </section>
      ) : projects.length === 0 ? (
          ...
          <section className="chapter-layout">
            <aside className="panel chapter-list">
              ...
              <div className="panel-body stack-list">
                {chaptersLoadedFor !== selectedNovelId ? (
                  <div className="inline-loading">正在读取章节…</div>
                ) : chapters.length === 0 ? (
            ...
            <form className={`panel chapter-editor ${contentExpanded ? "content-expanded" : ""} ${contextExpanded ? "context-expanded" : ""}`} onSubmit={saveChapter}>
              <div className="panel-header">
                ...
                <div className="inline-actions">
                  ...
                  <button className="primary-button" disabled={isChapterDetailLoading}>保存章节</button>
                </div>
              </div>
              <div className="panel-body form-grid">
                ...
                <section className="collapse-block chapter-content-block">
                  ...
                  {isChapterDetailLoading ? (
                    <div className="inline-loading" aria-busy="true">正在读取章节正文…</div>
                  ) : contentExpanded ? (
                ...
                <section className="collapse-block context-block">
                  ...
                  {isChapterDetailLoading ? (
                    <div className="inline-loading" aria-busy="true">正在读取上下文快照…</div>
...
export default function ChaptersPage() {
  return (
    <Suspense fallback={<div className="route-loading">正在载入章节管理...</div>}>
      <ChaptersContent />
    </Suspense>
  );
}
