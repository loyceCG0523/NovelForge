  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      setDisplay((current) => {
        ...
        const remaining = targetText.length - current.text.length;
        const charsPerFrame = remaining > 400 ? 8 : remaining > 80 ? 4 : 2;
        return {
          identity,
          text: targetText.slice(0, current.text.length + Math.min(charsPerFrame, remaining))
        };
      });
    });
    return () => window.cancelAnimationFrame(frame);
  }, [display.identity, display.text.length, identity, shouldAnimate, targetText]);
